...
# DBSCAN
# ─────────────────────────────────────────────
def dbscan_auto(X: np.ndarray) -> DBSCAN:
    """
    Estima eps via gráfico k-distance (k=5) e ajusta DBSCAN.
    """
    k = 5
    ...
    # Estima eps como o ponto de maior curvatura (heurística simples)
    diff  = np.diff(k_dists)
    joelho = int(np.argmax(diff < diff.mean() * 0.5))
    eps_est = float(k_dists[joelho])

    # Plot k-distance
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.plot(k_dists, color='steelblue')
    ax.axhline(eps_est, color='tomato', linestyle='--',
               label=f'eps estimado ≈ {eps_est:.3f}')
    ...
    model = DBSCAN(eps=eps_est, min_samples=k)
    model.fit(X)

    n_clusters = len(set(model.labels_)) - (1 if -1 in model.labels_ else 0)
    n_noise    = int(np.sum(model.labels_ == -1))
    print(f"     DBSCAN → eps={eps_est:.3f}, "
